 ...
 */
public class ClientAppForLeaveTestScript implements Runnable {
    ...
    private static boolean first = true; //added for client testing
	...
	public ClientAppForLeaveTestScript (boolean U, int p, String m, String t, String s, Semaphore clientLock) {
		udp = U;
		port = p;
        message = m;
        timeStamp = t;
    ...
    private static void run(ClientConnection connection, boolean udp, String server, int port) {
        ...
        String[] commandsList = {"p", "g", "g"};
        while (commandCounter < 3) {
        	//block that forces waiting for specified seconds after each command
        	if (commandCounter == 1) {
        		//wait 2 seconds before sending first gossip message
        		try {
        			System.out.printf("\nGoing to sleep for 2 seconds...\n");
					TimeUnit.SECONDS.sleep(2);
				} catch (InterruptedException e) {
					System.out.printf("Failed to put this thread to sleep properly, exiting.\n"
							+ "Try running the script again.\n");
					break;
				}
        	} else if (commandCounter == 2) {
        		//wait 7 seconds before sending the second gossip message
        		try {
        			System.out.printf("\nGoing to sleep for 7 seconds...\n");
    ...
    private static void gossip(Scanner in, ClientConnection connection, Date date) {
        ...
        String message = "";
        if (first) {
        	message = "First UDP test gossip message";
        	first = false;
        } else {
        	message = "Second UDP test gossip message";
        }
