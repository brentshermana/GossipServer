...
public class ClientAppForTCPTestScript {
    ...
    public static void main (final String[] args) {
    	...
    	System.out.println("This is the TCP test client.  The actual client takes in commands"
    			...
    			+ " start the server on the port that this TCP test client will automaticlaly attempt"
    			...
    			+ " send to the server)\n\nThe TCP test client will then exit after demonstrating"
    			+ " that every command works.\n\nUnlike the actual client, which is started via"
    			+ " the Master.java program along with the server, this test program is started on"
    			+ " its own so that it can more automatically demontrate the functionality of the"
    			+ " client.  However, all of the code in this test client is the exact same as the"
    			+ " code in the actual client, except for the code that handles accepting arguments"
    			+ " from the command line at the beginning of this program, and that the starting"
    			+ " block of code in this test program is in the \"main\" method, and not the run"
    			+ " method, which also is related to the actual client being started as a separate"
    			+ " thread by the Master class.  There are also some small differences in the code"
    			+ " made to make the running of this test client automated.\n\n");
    ...
    private static void run(ClientConnection connection, boolean udp, String server, int port) {
        ...
        connection.close();
        System.out.printf("\nThis testing program has now demonstrated all aspects of the TCP client,"
    ...
    private static void gossip(Scanner in, ClientConnection connection, Date date) {
    	...
    	if (date == null) {
            ...
            while (!valid) {
            	...
            	System.out.printf("For this testing, the date/time string will be:\n"
            			+ "02/18/2010 07:38:15:238\n");
            	String input = "02/18/2010 07:38:15:238";
                try {
        ...
        String message = "";
        if (initdated) {
        	message = "First TCP test gossip; message with unspecified date/time";
        } else {
        	message = "Second TCP test gossip; message with specified date/time";
        }
